...
- A single static binary with millisecond startup, suitable for per-turn agent invocation.
...
| --- | --- | --- | --- | --- |
| `config` | `Config::load(source: ConfigSource) -> Result<Config, Error>`; `Config { version, profiles, credentials }`; `Config::select_profile(cli_flag, env) -> Result<&Profile>` | Locate file (flag/env/XDG/platform default), parse TOML preserving key order, run strict core validation (descriptions, references, provider fields, reserved keys, sensitive-name rules, `inject` table shape), profile resolution order | `toml` (preserve_order), `serde` | Invariant: a returned `Config` is fully validated; downstream modules never re-check structure. Error modes: NotFound, Parse, Validation — each carries the offending config path string. |
...
| `render` | `render_text(view) -> String`; `render_json(view) -> serde_json::Value` | All user-facing formatting for query results, stable JSON shapes (top-level carries config `version`) | `serde_json` | Single choke point for output; JSON stability is a compatibility contract. |
| `credential` | trait `Provider { fn shallow_status(&self) -> Status; fn resolve(&self) -> Result<Secret>; fn store(&self, value) -> Result<()> }`; `Secret` newtype (no `Display`/`Debug` of contents) | The only module that touches secret values. Adapters: `EnvProvider`, `KeychainProvider` (keyring crate), `CommandProvider` (direct `argv` exec, no shell) | `keyring`, `std::process` | `Secret` redacts in Debug; `store` is `Err(Unsupported)` for env/command. Shallow status never resolves: env = getenv presence, keychain = "configured", command = argv[0] on PATH. |
...
| `cli` | `main()`, clap command tree, `Error -> exit code` mapping | Argument parsing, command dispatch, error rendering, exit codes 0/1/2/3/4 | all above | The only module that prints to stderr/stdout for errors. |
...
| JSON output | Agents | Top-level `version`, full paths, types, values; credentials as name+provider+status only | Stable contract (PRD-NFR-003) |
...
- Decision: Parse with the `toml` crate into typed core structs (version, profiles, credentials) + `toml::Value` for open user fields, `preserve_order` feature on.
...
- Decision: `Secret(String)` with manual `Debug`/no `Display`/no `Serialize`; constructed only inside `credential`, consumed only by `runner` (env injection) and `credential check` (discarded). `query`/`render` types cannot represent a secret.
...
- Automated tests: unit tests for `path` grammar, validation rules, injection-plan conflicts; integration tests for each CLI command including exit codes; keyring mock store for keychain flows; fake scripts for `command` provider.
- Manual validation: real macOS Keychain round-trip (`credential set` → `check` → `run`) once, recorded in validation.md.
