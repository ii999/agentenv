...
| `project` (facade) | `ProjectContext::resolve(cwd, env) -> Result<ProjectContext, AppError>`; `ProjectContext` = `None \| Untrusted { path, reason: UntrustedReason } \| Trusted { path, pin: Option<ProjectPin>, requires }`; `UntrustedReason` = `New \| Changed \| Invalid(Vec<Violation>) \| Unparseable \| StateUnavailable(String)` | Composition order: discover → canonicalize → trust-store load → path-only approval lookup → content read → fingerprint comparison → parse/validate. Fallible: a corrupt trust store and an unreadable file *with* an approval record are errors (exit 2). An unresolvable state base yields `Untrusted(StateUnavailable)`; an unreadable/unparseable/invalid file *without* an approval record yields `Untrusted` with the matching structured reason, which is what `project status` renders (violations included) and what the notice summarizes. | locate, model, trust | The single seam the rest of the CLI sees. Adapters: production (real fs/env) and tests (temp dirs + injected env), matching the existing `locate.rs` test style. |
