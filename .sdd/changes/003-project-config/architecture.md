...
| `project` (facade) | `ProjectContext::resolve(cwd, env) -> ProjectContext` = `None \| Untrusted { path, reason } \| Trusted { path, pin, requires }` | Compose locate → read → parse → trust check into the one value the CLI consumes. An unreadable/unparseable file yields `Untrusted` with the reason; content errors surface fully only via `project status` and `allow`. | locate, model, trust | The single seam the rest of the CLI sees. Adapters: production (real fs/env) and tests (temp dirs + injected env), matching the existing `locate.rs` test style. |
| `config::model::select_profile` | `select_profile(flag, env_val, project_pin: Option<&str>)` | Insert the trusted pin between `AGENTENV_PROFILE` and `default_profile` (PRD-FR-002). A pin naming an unknown profile is the existing unknown-profile error (exit 3) with the project file named in the message. | existing | All three call sites (`cli/mod.rs`, `config/write.rs` ×2) pass the pin from `ProjectContext`. |
| `cli::project` | `agentenv project status [--json]`, `agentenv project allow`, `agentenv project revoke` | `status`: full report (discovery result, trust state, pin, per-requirement satisfied/unsatisfied). `allow`: validate content, then record trust; refuses an invalid file. `revoke`: remove the record. | project, config | `status --json` is the agent surface; shape versioned like existing JSON output. |
...
| Untrusted-file notice | Humans (stderr) | One line naming the file and `agentenv project status`; emitted by commands that resolve configuration while an untrusted file is present | stderr only; never on stdout, never in `--json` payloads |
| Exit statuses | Agents, scripts | New status `5`: project file present but not trusted (used by `project status`/`allow` flows only); existing statuses keep their meanings | Documented extension of the README table |
...
- Consequences: new `sha2` dependency; moving a repo or re-cloning requires re-approval (accepted, matches direnv).
...
- Decision: while a discovered file is untrusted (new, changed, unreadable, or unparseable), all commands behave exactly as if it were absent, except for a single stderr notice naming the file and pointing at `agentenv project status`. Full diagnosis (parse errors, forbidden keys, requirement details) appears only in `project status` and `allow` output. A *trusted* file that later fails to read is an error (exit 2), never a silent skip.
...
- Decision: `project status` reports, per declared requirement, whether the entry exists in the active profile and whether each declared field path resolves within it. It performs no credential resolution and no shallow provider checks, and no other command is blocked by unsatisfied requirements.
