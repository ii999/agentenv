...
| `project::trust` | `TrustStore::load(env) -> Result<TrustStore, AppError>`; `lookup(canonical_path) -> Option<&Fingerprint>` (path-only, no content needed); `allow(canonical_path, content)`; `revoke(canonical_path)` | Persist approvals as `(canonical path, SHA-256 of content)` in a user-owned TOML state file. Path-only `lookup` lets the facade classify a trusted-but-unreadable file before any content read. Mutations are atomic (0600-first temp + rename); a corrupt store is a load error, never treated as empty. Filesystem operations go through an injectable seam (production: std fs; tests: fault-injecting adapter) so interrupted-rename and post-read-replacement cases (AC-003.11/AC-003.12) are unit-testable. | `sha2`, std fs | State file location: `$XDG_STATE_HOME/agentenv/trust.toml`, else `~/.local/state/agentenv/trust.toml`; Windows `%LOCALAPPDATA%\agentenv\trust.toml`. Never stored in the repo. |
| `project` (facade) | `ProjectContext::resolve(cwd, env) -> Result<ProjectContext, AppError>`; `project::allow(cwd, env) -> Result<AllowOutcome, AppError>` — facade-owned mutation performing discovery, the single content read, validation, fingerprinting, and the trust-store record over the same byte snapshot (the caller never re-reads); `project::revoke(cwd, env) -> Result<RevokeOutcome, AppError>` — discovery, canonicalization, and path-only record removal with no content read or validation, so changed, invalid, or unreadable files can always be revoked; `ProjectContext` = `None \| Untrusted { path, reason: UntrustedReason, meta: Option<ProjectFileMeta> } \| Trusted { path, meta: ProjectFileMeta }`; `UntrustedReason` = `New \| Changed \| Invalid(Vec<Violation>) \| StateUnavailable(String)`; `ProjectFileMeta` = `{ pin: Option<ProjectPin>, requires: Vec<Requirement> }` | Composition order over one immutable byte snapshot: discover → canonicalize → trust-store load → path-only approval lookup → single content read → classify (read failure with approval record ⇒ error exit 2; without ⇒ `Invalid` with a read-failure violation) → fingerprint the snapshot → compare → parse/validate the same bytes. `Invalid` covers schema violations, unparseable TOML, and unapproved-unreadable files (violations name paths and failure class only). For parseable `New`/`Changed` files `meta` carries the inert pin and requirement declarations so `project status` renders everything through this seam; behavioral consumers (selection, requirement checking) read `meta` only from `Trusted`. A corrupt trust store is an error (exit 2); an unresolvable state base yields `Untrusted(StateUnavailable)`. | locate, model, trust | The single seam the rest of the CLI sees — the status renderer never re-reads or re-parses the file. Adapters: production (real fs/env) and tests (temp dirs + injected env), matching the existing `locate.rs` test style. |
...
- Decision: while a discovered file is untrusted (new, changed, or — absent an approval record — unreadable or unparseable), all commands outside the `project` subcommand group behave exactly as if it were absent, except for a single stderr notice naming the file and pointing at `agentenv project status`. The notice is owned by a pre-dispatch prelude in the CLI entry path (`main.rs`): it runs after command-line parsing succeeds and before command dispatch, writes and flushes the notice to stderr, and hands the resolved `ProjectContext` to the command — which is what guarantees emission on success and failure paths alike and before `run`'s `exec` replaces the process. Invocations that fail CLI parsing, `--help`, `--version`, the no-subcommand help, `project` subcommands, and bypassed invocations never emit it. Command outcomes carry stdout, stderr, and exit status explicitly so a report and a non-zero status can coexist. Full diagnosis (parse errors, forbidden keys, requirement details) appears only in `project status` and `allow` output. A file with an approval record for its canonical path that later fails to read is an error (exit 2), never a silent skip; an unresolvable state base on read paths degrades the file to untrusted with the reason in the notice, while `allow`/`revoke` fail explicitly (exit 2).
...
- Decision: extend `Config::select_profile(flag, env_val)` to `select_profile(flag, env_val, project_pin)` and update its call sites that use the standard precedence chain, keeping the existing error catalogue. The `--create-profile` branch (`resolve_write_profile`) bypasses `select_profile` by accepted change-002 contract and is NOT wired to the pin: profile creation stays explicit-flag-only.
